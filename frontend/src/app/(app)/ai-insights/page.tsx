...
export default function AiInsightsPage() {
  ...
  const [opportunities, setOpportunities] = useState<InsightEntry[]>([]);
  ...
  function load() {
    ...
    listInsights({ category: "optimization", status: "new", page_size: 25 })
      .then((res) =>
        setOpportunities(
          [...res.items]
            .sort((a, b) => (b.estimated_production_impact_value ?? 0) - (a.estimated_production_impact_value ?? 0))
            .slice(0, 5)
        )
      )
      .catch(() => undefined);
    ...
    <div className="flex flex-1 flex-col gap-6">
      ...
      <div className="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-950">
        <div className="flex items-baseline justify-between gap-3">
          <h3 className="text-sm font-semibold text-zinc-900 dark:text-zinc-50">Optimization Opportunities</h3>
          <Link href="/ai-insights/list?category=optimization" className="text-xs text-zinc-500 hover:underline dark:text-zinc-400">
            View all →
          </Link>
        </div>
        {opportunities.length > 0 ? (
          ...
          <p className="mt-3 text-sm text-zinc-500 dark:text-zinc-400">No open optimization opportunities identified yet.</p>
        )}
      </div>
